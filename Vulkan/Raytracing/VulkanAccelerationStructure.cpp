...
namespace maple
{
	...
	VulkanAccelerationStructure::VulkanAccelerationStructure(uint64_t vertexAddress,
		...
		bool opaque)
	{
		...
		for (auto i = 0; i < subMeshes.size(); i++)        //TODO With subMeshes.
		{
			...
			geometry.geometry.triangles.maxVertex = subMeshes[i].numberOfVertex;
			...
			maxPrimitiveCounts.push_back(subMeshes[i].indexCount / 3);

			VkAccelerationStructureBuildRangeInfoKHR buildRange{};
			buildRange.primitiveCount = subMeshes[i].indexCount / 3;
			buildRange.primitiveOffset = subMeshes[i].indexOffset * sizeof(uint16_t);
		...
		MAPLE_ASSERT(!geometries.empty(), "error");
