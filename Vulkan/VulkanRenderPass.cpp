...
namespace maple
{
	...
	auto VulkanRenderPass::beginRenderPass(const CommandBuffer* commandBuffer, const float* clearColor, FrameBuffer* frame, SubPassContents contents, uint32_t width, uint32_t height, int32_t cubeFace, int32_t mipMapLevel) const -> void
	{
		PROFILE_FUNCTION();
		if (!depthOnly)
		{
			...
			{
				clearValue[i].color.float32[0] = clearColor[0];
				clearValue[i].color.float32[1] = clearColor[1];
				clearValue[i].color.float32[2] = clearColor[2];
				clearValue[i].color.float32[3] = clearColor[3];
			}
		}

		if (clearDepth)
		{
			clearValue[clearCount - 1].depthStencil = VkClearDepthStencilValue{ 0.0f, 0 };
		}
	...
	auto VulkanRenderPass::beginRenderPass(const CommandBuffer* commandBuffer, const float* clearColor, FrameBuffer* frame, SubPassContents contents, uint32_t width, uint32_t height, const int32_t * viewport) const -> void
	{
		...
		if (viewport[0] >= 0)
			commandBuffer->updateViewport(viewport[0], viewport[1], viewport[2], viewport[3]);
		else
			commandBuffer->updateViewport(width, height);
	}
