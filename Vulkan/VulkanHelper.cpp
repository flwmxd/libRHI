...
namespace maple
{
	namespace
	{
		...
		inline auto layoutToAccessMask(const VkImageLayout layout, const bool isDestination) -> VkPipelineStageFlags
		{
			...
			case VK_IMAGE_LAYOUT_UNDEFINED:
				if(isDestination) { LOGE("The new layout used in a transition must not be VK_IMAGE_LAYOUT_UNDEFINED."); }
				break;
			...
			case VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL: accessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT; break;
			...
			default: LOGE("Unexpected image layout"); break;
			}
		...
		inline auto accessFlagsToPipelineStage(VkAccessFlags accessFlags, const VkPipelineStageFlags stageFlags)
		{
			...
			while(accessFlags != 0) {
				...
				default: LOGE("Unknown access flag"); break;
				}
			}
			return stages;
		}
	...
	auto VulkanHelper::validateResolution(uint32_t &width, uint32_t &height) -> void
	{
		...
		auto surface = std::static_pointer_cast<VulkanSwapChain>(GraphicsContext::get()->getSwapChain())->getSurface();
	...
	auto VulkanHelper::instanceCreateInfo(const VkApplicationInfo &appInfo, const std::vector<const char *> &extensions,
	                                      const std::vector<const char *> &validationLayers, bool validationLayer) -> VkInstanceCreateInfo
	{
		// Create Info
	...
	auto VulkanHelper::createBuffer(VkBuffer &buffer, VkDeviceMemory &bufferMemory, VkPhysicalDevice physicalDevice, VkDevice device, VkDeviceSize size,
	                                VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBufferCreateFlags flags /*= 0*/,
...
#undef STR
	...
	{
		if(stencil) {
			return getSupportedFormat({VK_FORMAT_D24_UNORM_S8_UINT, VK_FORMAT_D32_SFLOAT_S8_UINT}, VK_IMAGE_TILING_OPTIMAL,
			                          VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
		}
		return getSupportedFormat({VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D32_SFLOAT, VK_FORMAT_D24_UNORM_S8_UINT}, VK_IMAGE_TILING_OPTIMAL,
		                          VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
	}
	...
	auto VulkanHelper::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities, uint32_t width, uint32_t height) -> VkExtent2D
	{
	...
	auto VulkanHelper::createImageView(VkImage image, VkFormat format, uint32_t mipLevels, VkImageViewType viewType, VkImageAspectFlags aspectMask,
	                                   uint32_t layerCount, uint32_t baseArrayLayer, uint32_t baseMipLevel) -> VkImageView
	{
		PROFILE_FUNCTION();
		...
		if(vkCreateImageView(*VulkanDevice::get(), &viewInfo, nullptr, &imageView) != VK_SUCCESS) { LOGE("Failed to create texture image view!"); }
...
#else
	auto VulkanHelper::createImageDefault(const VkImageCreateInfo &imageInfo, VkImage &image, VkDeviceMemory &imageMemory, VkMemoryPropertyFlags properties)
	    -> void
	{
		PROFILE_FUNCTION();
...
#ifdef USE_VMA_ALLOCATOR
	auto VulkanHelper::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageType imageType, VkImageTiling tiling,
	                               VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage &image, VkDeviceMemory &imageMemory,
	                               uint32_t arrayLayers, VkImageCreateFlags flags, VmaAllocation &allocation, uint32_t depth, VkImageLayout initLayout,
	                               void *next) -> void
#else
	auto VulkanHelper::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageType imageType, VkImageTiling tiling,
	                               VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage &image, VkDeviceMemory &imageMemory,
	                               uint32_t arrayLayers, VkImageCreateFlags flags, uint32_t depth, VkImageLayout initLayout, void *next) -> void
#endif
	{
		PROFILE_FUNCTION();
	...
	auto VulkanHelper::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels,
	                                         uint32_t layerCount, const VulkanCommandBuffer *cmd, bool depth, uint32_t baseArrayLayer,
	                                         uint32_t baseMipLevel) -> void
	{
		PROFILE_FUNCTION();
		...
		if(isStencilFormat(format)) subresourceRange.aspectMask |= VK_IMAGE_ASPECT_STENCIL_BIT;
		...
		if(singleTimeCommand) endSingleTimeCommands(commandBuffer);
	}
	...
	auto VulkanHelper::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, uint32_t depth, int32_t offsetX, int32_t offsetY,
	                                     int32_t offsetZ, const VulkanCommandBuffer *cmd) -> void
	{
		...
		if(cmd == nullptr) endSingleTimeCommands(commandBuffer);
	}

	VKAPI_ATTR auto VKAPI_CALL VulkanHelper::debugUtilsMessengerCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
	...
	auto VulkanHelper::setImageLayout(VkCommandBuffer cmdbuffer, VkImage image, VkImageLayout oldImageLayout, VkImageLayout newImageLayout,
	...
	auto VulkanHelper::setImageLayout(VkCommandBuffer cmdbuffer, VkImage image, VkImageAspectFlags aspectMask, VkImageLayout oldImageLayout,
	                                  VkImageLayout newImageLayout, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask) -> void
	{
		VkImageSubresourceRange subresourceRange = {};
	...
	auto VulkanHelper::createTextureSampler(VkFilter magFilter /*= VK_FILTER_LINEAR*/, VkFilter minFilter /*= VK_FILTER_LINEAR*/, float minLod /*= 0.0f*/,
	                                        float maxLod /*= 1.0f*/, bool anisotropyEnable /*= false*/, float maxAnisotropy /*= 1.0f*/,
	                                        ...
	                                        VkSamplerAddressMode modeW /*= VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE*/) -> VkSampler
	{
		PROFILE_FUNCTION();
		VkSampler sampler;
	...
	namespace VkConverter
	{
		auto textureFormatToVK(const TextureFormat &format, bool srgb, bool ignoreAssert) -> VkFormat
		{
			if(srgb) {
				...
				default: MAPLE_ASSERT(ignoreAssert, "[Texture] Unsupported image bit-depth!"); return VK_FORMAT_UNDEFINED;
				}
			} else {
				switch(format) {
				...
				case TextureFormat::R16: return VK_FORMAT_R16_SFLOAT;
				...
				case TextureFormat::R32F: return VK_FORMAT_R32_SFLOAT;
				case TextureFormat::R16F: return VK_FORMAT_R16_SFLOAT;
				case TextureFormat::R11G11B10: return VK_FORMAT_R16G16B16A16_SFLOAT;
				default: MAPLE_ASSERT(ignoreAssert, "[Texture] Unsupported image bit-depth!"); return VK_FORMAT_UNDEFINED;
				}
			}
		}
		...
		auto textureMipmapFilterToVK(TextureFilter filter) -> VkSamplerMipmapMode
		{
			...
			case TextureFilter::None: return VK_SAMPLER_MIPMAP_MODE_LINEAR;
			default: LOGC("[Texture] Unsupported TextureFilter type!"); return VK_SAMPLER_MIPMAP_MODE_LINEAR;
			}
		}
		...
		{
			switch(type) {
			case DrawType::Triangle: return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
			case DrawType::Lines: return VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
			case DrawType::Point: return VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
			case DrawType::TriangleStrip: return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;
			case DrawType::Lines_Strip: return VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
			case DrawType::Triangle_Fan: return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN;
			default: LOGC("Unknown Draw Type"); return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
			}
		}
		...
		{
			switch(mode) {
			case PolygonMode::Fill: return VK_POLYGON_MODE_FILL; break;
			case PolygonMode::Line: return VK_POLYGON_MODE_LINE; break;
			case PolygonMode::Point: return VK_POLYGON_MODE_POINT; break;
			default:
		...
		auto descriptorTypeToVK(DescriptorType type) -> VkDescriptorType
		{
			...
			case DescriptorType::Buffer: return VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
			case DescriptorType::BufferDynamic: return VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		...
		{
			constexpr VkShaderStageFlagBits types[12] = {VK_SHADER_STAGE_VERTEX_BIT,
			...
			const ShaderType flags[11] = {ShaderType::Vertex,
			                              ShaderType::Fragment,
			                              ShaderType::Geometry,
			...
			for(auto i = 0; i < 11; i++) {
				if(((uint32_t)shaderName & (uint32_t)flags[i]) == (uint32_t)flags[i]) { retBits |= types[i]; }
			}

			if(retBits != 0) return (VkShaderStageFlagBits)retBits;
