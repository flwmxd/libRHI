...
namespace maple
{
	class VkTexture
	{
	  public:
		...
		virtual auto getImageLayout(int32_t mipLevel = -1) const -> VkImageLayout                                          = 0;
	...
	class VulkanTexture2D : public Texture2D, public VkTexture
	{
	  public:
		...
		inline auto getImageLayout(int32_t mipLevel = -1) const -> VkImageLayout override
		{
			return mipLevels > 1 && mipLevel >= 0 ? imageLayouts[mipLevel] : imageLayout;
		}
		...
		auto buildPyramid(TextureFormat internalformat, uint32_t width, uint32_t height) -> void override;
	  ...
	  private:
		...
		auto deleteSampler(bool delSampler = true) -> void;
	...
	class VulkanTextureDepth : public TextureDepth, public VkTexture
	{
	  public:
		...
		auto readBarrier(const CommandBuffer *cmd) -> void override;
		auto writeBarrier(const CommandBuffer *cmd) -> void override;
