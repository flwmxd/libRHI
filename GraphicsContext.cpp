...
namespace maple
{
	...
	auto Shader::create(const std::vector<uint32_t>& compData, const VariableArraySize& size /*= {}*/) ->std::shared_ptr<Shader>
	{
#ifdef MAPLE_VULKAN
		return std::make_shared<VulkanShader>(compData, size);
#endif

#ifdef MAPLE_OPENGL
		return io::emplace<GLShader>(compData, size);
#endif
	}

	auto Shader::create(const std::vector<uint32_t> &vertData, const std::vector<uint32_t> &fragData,
	                    const std::unordered_set<std::string> &dynamicUniforms) -> std::shared_ptr<Shader>
	{
#ifdef MAPLE_VULKAN
	...
	auto Shader::create(const ShaderTypes& shaderTypes, const VariableArraySize& size /*= {}*/, const std::unordered_set<std::string>& dynamicUniforms ) ->std::shared_ptr<Shader>
	{
#ifdef MAPLE_VULKAN
		return std::make_shared<VulkanShader>(shaderTypes, size, dynamicUniforms);
#endif
#ifdef MAPLE_OPENGL
		return std::make_shared<GLShader>(shaderTypes, size, dynamicUniforms);
#endif
	}
	...
	auto FrameBuffer::create(const FrameBufferInfo &desc) -> std::shared_ptr<FrameBuffer>
	{
		...
		return frameBufferCache
		    .emplace(std::piecewise_construct, std::forward_as_tuple(hash), std::forward_as_tuple(fb, Timer::get().currentTimestamp()))
		    .first->second.asset;
#endif
#ifdef MAPLE_OPENGL
	...
	auto Pipeline::get(const PipelineInfo &desc) -> std::shared_ptr<Pipeline>
	{
		...
		return pipelineCache
		    .emplace(std::piecewise_construct, std::forward_as_tuple(hash), std::forward_as_tuple(pipeline, Application::getTimer().currentTimestamp()))
		    .first->second.asset;
#endif // MAPLE_OPENGL
		...
		return pipelineCache
		    .emplace(std::piecewise_construct, std::forward_as_tuple(hash), std::forward_as_tuple(pipeline, Timer::get().currentTimestamp()))
		    .first->second.asset;
#endif // MAPLE_OPENGL
	}
	...
	auto RenderPass::create(const RenderPassInfo &desc) -> Ptr
	{
		...
		for(uint32_t i = 0; i < desc.attachments.size(); i++) { hash::hashCode(hash, desc.attachments[i]); }
		...
		return renderPassCache.emplace(hash, RenderPassAsset{std::make_shared<VulkanRenderPass>(desc), Timer::get().currentTimestamp()})
	...
	auto VertexBuffer::createRaw(const BufferUsage &usage) -> VertexBuffer * { return new VulkanVertexBuffer(usage); }

	auto VertexBuffer::createRaw(const void *data, uint32_t size, bool gpuOnly) -> VertexBuffer * { return new VulkanVertexBuffer(data, size, gpuOnly); }
