...
namespace maple
{
	...
	//###################################################
	auto Texture2D::createRaw()->Texture2D*
	{
#ifdef MAPLE_OPENGL
	...
	auto Texture2D::getTexture1X1Black()->std::shared_ptr<Texture2D> 
	{
	...
	auto TextureDepth::createRaw(uint32_t width, uint32_t height, bool stencil, const CommandBuffer* commandBuffer) -> TextureDepth*
	{
#ifdef MAPLE_OPENGL
		return new GLTextureDepth(width, height, stencil);
#endif        // MAPLE_OPENGL
#ifdef MAPLE_VULKAN
		return new VulkanTextureDepth(width, height, stencil, commandBuffer);
#endif        // MAPLE_VULKAN
	}
